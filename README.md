...
### Implémentées ✅
- **Authentification** par email + mot de passe (sessions par cookie signé HMAC, 30 jours)
- **2 rôles** :
...
- **Fiches piscines détaillées** : adresse géolocalisée, type, volume, forme, traitement, filtration, code d'accès, notes d'accès (chien/portail/produits), **routine d'entretien** (checklist), notes
...
- **Agenda** :
  ...
  - **Vue Agenda (liste)** et **vue Carte** avec parcours numéroté tracé sur la carte
  - Filtre par intervenant (admin)
- **Vue intervenant** : Romain ne voit que ses entretiens et ses piscines, avec checklist de routine interactive, code d'accès mis en avant, contact client cliquable, mini-carte
- **Marquage des passages effectués** (historique)

### À envisager plus tard 🔜
- Photos de piscines (upload via Cloudflare R2)
- Optimisation automatique de l'ordre du parcours (tournée optimale)
- Notifications / rappels
- Gestion fine des produits et dosages
...
| POST | `/api/maintenances/:id/log` | Marquer un passage effectué |
